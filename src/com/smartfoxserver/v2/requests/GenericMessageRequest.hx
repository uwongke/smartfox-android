	...
	private function executeObjectMessage(sfs:SmartFox):Void {
		...
		if (Std.isOfType(_recipient, Array)) {
			...
			if (potentialRecipients.length > _room.capacity)
				throw new ArgumentError("The number of recipients is bigger than the target Room capacity:"
					+ potentialRecipients.length);
			...
			for (item in potentialRecipients) {
				if (Std.isOfType(item, User))
					...
					sfs.logger.warn("Bad recipient in DynamicMessage recipient list:"
						+ Type.typeof(item)
						+ ", expected type:User");
			}
		}
